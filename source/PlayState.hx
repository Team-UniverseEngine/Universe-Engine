...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		if (ClientPrefs.data.ueresultscreen && seenResults)
		{
			if (controls.ACCEPT)
			{
	...
	{
		if (ClientPrefs.data.noteSplashes && !SONG.disableSplashes && note != null)
			{
		...
		for (i in 0...achievesToCheck.length)
		{
			...
			if (!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled)
			{
				...
				switch (achievementName)
				{
					...
					case 'toastie':
						if (/*ClientPrefs.framerate <= 60 &&*/ !ClientPrefs.data.shaders && ClientPrefs.data.lowQuality && !ClientPrefs.data.antialiasing)
						{
							unlock = true;
						}
					case 'debugger':
